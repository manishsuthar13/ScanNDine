...
const Auth = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'login') {
        const res = await api.login({ email: form.email, password: form.password, role });  // Added role
        ...
        if (role === 'admin') {
          const checkRes = await api.checkAdminExists();
          if (checkRes.data.exists) return setError('Admin already exists. Contact support.');
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-100 to-green-100 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
        ...
        {role === 'customer' && <button onClick={() => window.location.href = '/menu'} className="w-full bg-blue-500 text-white py-3 rounded mt-4 hover:bg-blue-600">Continue as Guest</button>}
      </div>
    </div>
  );
};
